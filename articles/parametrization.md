...
$$g(y) = (2\pi\tau)^{-1/2}y^{-1}{\exp}\left\{ -\frac{({\log}y - \eta)^{2}}{2\tau} \right\}.$$
The expected value (theoretical mean) of such a variable is
$$E\lbrack Y\rbrack = {\exp}\{\eta + \tau/2\},$$ while the variance is:
$$V\lbrack Y\rbrack = \lbrack{\exp}\{\tau\} - 1\rbrack{\exp}\{ 2\eta + \tau\}.$$
...
$$\phi = \lbrack{\exp}\{\tau\} - 1\rbrack{\exp}\{ 2\eta + \tau\}.$$
...
following density:
$$g(y) = {(\pi\phi)}^{-1/2}y^{-1}{\exp}\left\{ -\frac{1}{\phi}{({\log}y - {\log}\mu + \phi)}^{2} \right\}.$$
