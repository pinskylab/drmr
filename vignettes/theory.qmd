...
|:----------:|:---------:|:---------------------------------------------:|
| $\beta_t$  | `beta_t`  | Reg. coef. for proba. of absence              |
| $\beta_r$  | `beta_r`  | Reg. coef. for recruitment                    |
| $\phi$     | `phi`     | Dispersion parameter                          |
| $\beta_s$  | `beta_s`  | Reg. coef. for survival                       |
| $\xi$      | `xi`      | $\mathrm{logit}(\rho) = \cdots \xi \log(\mu)$ |
| $\zeta$    | `zeta`    | Prob. of remaining in the same patch          |
| $\alpha$   | `alpha`   | AR(1) temporal correlation                    |
| $\sigma_t$ | `sigma_t` | AR(1) conditional SD                          |
| $z_t$      | `z_t`     | AR(1) random effects                          |
| $\sigma_i$ | `sigma_i` | Patch level iid random effect SD              |
| $z_i$      | `z_i`     | Patch level iid random effects                |
| $\sigma_s$ | `sigma_s` | Patch level ICAR random effect SD             |
| $z_s$      | `z_s`     | Patch level ICAR random effects               |
...
|:----------:|:---------:|--------------------------------------|:-----------------------------:|
| $\beta_t$  | `beta_t`  | $\mathrm{N}(m_t, s_t)$               | $m_t = 0, s_t = 1$            |
| $\beta_r$  | `beta_r`  | $\mathrm{N}(m_r, s_r)$               | $m_r = 0, s_r = 1$            |
| $\phi$     | `phi`     | $\mathrm{Gamma}(a_p, b_p)$           | $a_p = 2, b_p = 1$            |
| $\beta_s$  | `beta_s`  | $\mathrm{N}(m_s, s_s)$               | $m_s = 0, s_s = 1$            |
| $\xi$      | `xi`      | ${- \rm LN}(m_lxi, s_lxi)$           | $m_{lxi} = 0, s_{lxi} = 1$    |
| $\zeta$    | `zeta`    | $\mathrm{Beta}(a_z, b_z)$            | $a_z = .5, b_z = .5$          |
| $\alpha$   | `alpha`   | $\mathrm{Beta}(a_a, b_a)$            | $a_a = .5, b_a = .5$          |
| $\sigma_t$ | `sigma_t` | $\mathrm{LN}(m_lst, s_lst)$          | $m_{lst} = -2, s_{lst} = .25$ |
| $z_t$      | `z_t`     | $z_t \sim AR(1)$                     | N/A                           |
| $\sigma_i$ | `sigma_i` | $\mathrm{LN}(m_lsi, s_lsi)$          | $m_{lsi} = -2, s_{lsi} = .25$ |
| $z_i$      | `z_i`     | $z_i \sim \mathrm{N}(0, \sigma^2_i)$ | N/A                           |
| $\sigma_s$ | `sigma_s` | $\mathrm{LN}(m_lss, s_lss)$          | $m_{lss} = -2, s_{lss} = .25$ |
| $z_s$      | `z_s`     | $z_s \sim \mathrm{N}(0, \sigma^2_i)$ | N/A                           |
