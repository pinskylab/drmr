...
|:---------:|:---------:|:-----------------------------------------:|:------------------------:|-----------------------------------|:-------------------------:|
| $\beta_t$ | `beta_t`  | Reg. coef. for proba. of absence          | N/A                      | ${\rm N}(m_t, s_t)$               | $m_t = 0, s_t = 1$        |
| $\beta_r$ | `beta_r`  | Reg. coef. for recruitment                | N/A                      | ${\rm N}(m_t, s_t)$               | $m_r = 0, s_r = 1$        |
| $\phi$    | `phi`     | Dispersion parameter                      | N/A                      | ${\rm Gamma}(a_p, b_p)$           | $a_p = 2, b_p = 1$        |
| $\beta_s$ | `beta_s`  | Reg. coef. for survival                   | `est_surv`               | ${\rm N}(m_s, s_s)$               | $m_s = 0, s_s = 1$        |
| $\xi$     | `xi`      | ${\rm logit}(\rho) = \cdots \xi \log(\mu) | `rho_mu`                 | ${- \rm LN}(m_lxi, s_lxi)$        | $m_lxi = 0, s_lxi = 1$    |
| $\zeta$   | `zeta`    | Prob. of remaining in the same patch      | `movement`               | ${\rm Beta}(a_z, b_z)$            | $a_z = .5, b_z = .5$      |
| $\alpha$  | `alpha`   | AR(1) temporal correlation                | `ar_re` $\neq$ `"none"`  | ${\rm Beta}(a_a, b_a)$            | $a_a = .5, b_a = .5$      |
| $\sigma_t | `sigma_t` | AR(1) conditional SD                      | `ar_re` $\neq$ `"none"`  | ${\rm LN}(m_lst, s_lst)$          | $m_lst = -2, s_lst = .25$ |
| $z_t      | `z_t`     | AR(1) random effects                      | `ar_re` $\neq$ `"none"`  | $z_t \sim AR(1)$                  | N/A                       |
| $\sigma_i | `sigma_i` | Patch level iid random effect SD          | `iid_re` $\neq$ `"none"` | ${\rm LN}(m_lsi, s_lsi)$          | $m_lsi = -2, s_lsi = .25$ |
| $z_i      | `z_i`     | Patch level iid random effects            | `iid_re` $\neq$ `"none"` | $z_i \sim {\rm N}(0, \sigma^2_i)$ | N/A                       |
| $\sigma_s | `sigma_s` | Patch level ICAR random effect SD         | `sp_re` $\neq$ `"none"`  | ${\rm LN}(m_lss, s_lss)$          | $m_lss = -2, s_lss = .25$ |
| $z_s      | `z_s`     | Patch level ICAR random effects           | `sp_re` $\neq$ `"none"`  | $z_s \sim {\rm N}(0, \sigma^2_i)$ | N/A                       |
