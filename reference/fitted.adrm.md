...
Calculates the "fitted values" associated to an `adrm` object. These
fitted values are useful for "hindcasts" and posterior predictive
checks.
...
- object:
  ...
  `adrm` object containing the output from the
...
- cores:
  ...
  the `fit_drm`, then four (or less) threads are recommended.
